...
export default function CoinDetails(){
  ...
  const futurePadding = Array(horizon).fill(null);
  const historyPadded = [...historyOnly, ...futurePadding];
  ...
  }, [predictions, historyOnly, horizon]);
  ...
  async function handlePredict(){
    ...
    try {
      ...
      const { xsT, ysT, min, max } = makeWindowedDataset(rawSeries, windowSize);
      const model = buildModel(windowSize);

      await train(model, xsT, ysT, 12);
      const preds = forecast(model, rawSeries, windowSize, min, max, horizon);
  ...
  // ----- Conditional UI (no early returns) -----
  let content;
  if (loading) {
    content = <p>Loading coin details…</p>;
  } else if (!coin) {
    content = <p>Coin not found.</p>;
  } else {
    const data = {
      labels,
      datasets: [
